...
*/
    ...
    #define SEND_FUNCTION(soc, buf, len, flags) send((LONG)soc, (const UBYTE*)buf, (LONG)len, (LONG)flags)
    #define RECV_FUNCTION(soc, buf, len, flags) recv((LONG)soc, (UBYTE*)buf, (LONG)len, (LONG)flags)
#else
...
#ifdef __MORPHOS__
#undef SocketBase
#define SocketBase ssl->socketbase
...
#ifdef __MORPHOS__
    if(!SocketBase) {
        CYASSL_MSG("CyaSSL CTX SocketBase == NULL");
